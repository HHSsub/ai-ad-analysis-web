...
export class AutomationScheduler {
  ...
  start() {
    ...
    // Drive 자동 업로드도 시작
    this.driveUploader.startAutoCleanup();
  }
  ...
  stop() {
    ...
    // Drive 자동 업로드 중지
    this.driveUploader.stopAutoCleanup();
  }
  ...
  getStatus() {
    return {
      ...
      nextExecutions: this.tasks.map((task, index) => ({
        name: this.getTaskName(index),
        isRunning: this.isRunning
      }))
    };
  }
  ...
  private getTaskName(index: number): string {
    const names = ['광고수집', '분석전송', 'Drive업로드', '시스템체크'];
    return names[index] || `작업${index}`;
  }
